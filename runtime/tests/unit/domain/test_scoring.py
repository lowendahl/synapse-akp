...
Validates component promises from docs/components/hybrid-search.md:
- P-FORMULA: RRF score = Σ 1/(k + rank)
- P-DEDUP: Same hit across channels → merged, scores summed
- P-BOOST: Graph boost only affects 1-hop neighbors of top-N
- P-BOOST-SCOPE: Boost seeds are not recursive
- P-CHANNEL-INVARIANT: Channel order doesn't affect output
- P-MONOTONIC: Adding channel never decreases score
- P-DETERMINISTIC: Same inputs → same outputs
- P-EMPTY-SAFE: Empty input → empty output
- INV-IMMUTABLE-INPUT: Inputs not mutated
"""
...
def test_rrf_fuse_empty_channels_returns_empty() -> None:
    assert rrf_fuse([[], [], []]) == []


# ─── P-FORMULA: RRF scoring math ────────────────────────────────────────────
...
def test_rrf_fuse_rank_is_one_based() -> None:
    """Second item in a channel gets rank=2, so score = 1/(k+2)."""
    hit1 = make_search_hit(object_id="first", score=0.0)
    hit2 = make_search_hit(object_id="second", score=0.0)

    fused = rrf_fuse([[hit1, hit2]], rrf_k=60)
    by_id = {h.object_id: h for h in fused}

    assert by_id["first"].score == pytest.approx(1 / 61)
    assert by_id["second"].score == pytest.approx(1 / 62)


# ─── P-DEDUP: Deduplication across channels ──────────────────────────────────
...
def test_rrf_fuse_dedup_key_is_pack_object_unit() -> None:
    """Same pack+object+unit in 2 channels → one output hit."""
    hit_a = make_search_hit(pack_id="p", object_id="o", unit_id="u", score=0.0)
    hit_b = make_search_hit(pack_id="p", object_id="o", unit_id="u", score=0.0)

    fused = rrf_fuse([[hit_a], [hit_b]], rrf_k=60)
    assert len(fused) == 1
    assert fused[0].score == pytest.approx((1 / 61) + (1 / 61))


# ─── P-BOOST: Graph boost behavior ──────────────────────────────────────────
...
def test_apply_graph_boost_empty_adjacency_no_change() -> None:
    """No adjacency → no boost applied."""
    hit = make_search_hit(object_id="x", score=0.8)
    boosted = apply_graph_boost([hit], adjacency={}, top_n=1, boost_value=0.2)
    assert boosted[0].score == pytest.approx(0.8)


def test_apply_graph_boost_top_n_limits_seeds() -> None:
    """P-BOOST-SCOPE: Only top-N (by score) seed the neighbor set."""
    top = make_search_hit(object_id="top", score=1.0)
    mid = make_search_hit(object_id="mid", score=0.5)
    low = make_search_hit(object_id="low", score=0.3)

    # mid's neighbor is low, but mid is NOT in top-1 → low should NOT be boosted
    boosted = apply_graph_boost(
        [top, mid, low],
        adjacency={"mid": {"low"}, "top": set()},
        top_n=1,
        boost_value=0.2,
    )
    by_id = {h.object_id: h for h in boosted}
    assert by_id["low"].score == pytest.approx(0.3)


# ─── P-CHANNEL-INVARIANT: Order independence ────────────────────────────────
...
# ─── P-MONOTONIC: Adding channel never lowers score ─────────────────────────
...
def test_rrf_fuse_does_not_mutate_input_lists() -> None:
    channel = [make_search_hit(score=0.0)]
    channels = [channel]
    original_len = len(channel)

    rrf_fuse(channels, rrf_k=60)

    assert len(channel) == original_len


# ─── Stable sort (already passing) ──────────────────────────────────────────
